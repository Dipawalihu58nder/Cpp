...
int main() {
    ...
    while (std::getline(infile, private_key_hex)) {
        try {
            ...
            std::cout << "Generated address: " << bitcoin_address << std::endl;
        } catch (const std::exception& e) {
            std::cerr << "Error processing key: " << private_key_hex << " - " << e.what() << std::endl;
        }
    }
    ...
    std::cout << "Finished generating addresses." << std::endl;
    return 0;
}
